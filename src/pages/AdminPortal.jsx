...
const AdminPortal = ({ setIsAdmin, setCurrentPage }) => {
    ...
    reader.onload = async (evt) => {
        try {
            ...
            const existingSerials = new Set(students.map(s => String(s.serialNumber || '').trim().toLowerCase()).filter(Boolean));

            data.forEach((row, index) => {
                const rowNum = index + 2; // Excel row (1-header)
                const name = row['Student Name'] || row['Name'];
                const serial = row['Serial No'] || row['Serial Number'];
                const providedId = row['Student ID'] || row['Member ID'];
                const grade = row['Class'] || row['Grade'] || selectedClassForList || ''; // Default to selected class if not in row
                ...
                if (serial) {
                    ...
                    if (existingSerials.has(serialStr.toLowerCase())) {
                    ...
                    if (processedSerials.has(serialStr.toLowerCase())) {
                        errors.push(`Row ${rowNum}: Duplicate Serial Number '${serial}' in import file.`);
                        return;
                    }
                ...
                if (!studentId) {
                    ...
                    // Check if ID exists
                    const idExists = students.some(s => s.id === studentId) || newStudents.some(s => s.id === studentId);
                    if (idExists) {
                ...
                const newStudent = {
                    id: studentId,
                    serial_number: serial ? String(serial).trim() : null, // Store Serial Number
                    name: name,
                    password: row['Password'] || ('acs' + Math.floor(1000 + Math.random() * 9000)),
                    grade: grade,
                    image: '', // No image import support from Excel usually
                    fee_status: 'unpaid',
                    results: [],
                    attendance: { present: 0, absent: 0, total: 0, percentage: 0 },
                    previous_results: [],
    ...
    reader.onload = async (evt) => {
        try {
            ...
            if (newStudents.length > 0) {
                const { error } = await addStudents(newStudents);
                if (error) {
                    alert("Database Error: " + error.message);
                } else {
                    let msg = `Successfully imported ${successCount} students.`;
                    if (errors.length > 0) {
                        msg += `\n\n${errors.length} rows failed:\n` + errors.slice(0, 10).join('\n') + (errors.length > 10 ? '\n...' : '');
                    }
                    alert(msg);
                    showSaveMessage(`${successCount} students imported!`);
                    if (classImportFileRef.current) classImportFileRef.current.value = '';
                }
            } else {
                alert("No valid students were imported.\n\nErrors:\n" + errors.join('\n'));
            }

        } catch (err) {
            console.error("Import Error:", err);
            alert("Failed to process Excel file. Please ensure it uses the correct format.");
        }
    };
    reader.readAsBinaryString(file);
};
